...
ReseauNeuronal* creer_reseau(int n_entrees, int n_couches, const int *taille_couches) {
    log_message(LOG_LEVEL_INFO, "Création d'un réseau avec %d couches et %d entrées.", n_couches, n_entrees);
...
void detruire_reseau(ReseauNeuronal *reseau) {
    log_message(LOG_LEVEL_INFO, "Destruction du réseau neuronal.");
...
void propagation(ReseauNeuronal *reseau, const double *entrees) {
    log_propagation_once();
    if (!reseau || !entrees) { 
        log_message(LOG_LEVEL_WARNING, "Propagation annulée : réseau ou entrées null.");
        return;
    }
